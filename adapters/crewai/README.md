...
CrewAI (https://crewai.com) is Multi-agent orchestration framework focused on role-based agents and tasks.

This adapter would let any Loopfile run as a crewai workflow, with
...
- **It proves the portability claim.** A Loopfile that runs on both
  Hermes and CrewAI is the demonstration that INFINI is real.
...
5. Run `infini conformance tests/conformance/ --engine=crewai` to verify.
...
└── examples/
    └── crewai-loop.yaml   ← at least one runnable example Loopfile
```
...
CrewAI adapter. Your name goes in CONTRIBUTORS.md.
