...
Mastra (https://mastra.ai) is TypeScript-first agent framework with built-in workflows and memory.

This adapter would let any Loopfile run as a mastra workflow, with
...
- **It proves the portability claim.** A Loopfile that runs on both
  Hermes and Mastra is the demonstration that INFINI is real.
...
5. Run `infini conformance tests/conformance/ --engine=mastra` to verify.
...
└── examples/
    └── mastra-loop.yaml   ← at least one runnable example Loopfile
```
...
Mastra adapter. Your name goes in CONTRIBUTORS.md.
