...
Goose (https://github.com/block/goose) is Open-source local AI agent from Block. Desktop-first, tool-heavy.

This adapter would let any Loopfile run as a goose workflow, with
...
- **It proves the portability claim.** A Loopfile that runs on both
  Hermes and Goose is the demonstration that INFINI is real.
...
5. Run `infini conformance tests/conformance/ --engine=goose` to verify.
...
└── examples/
    └── goose-loop.yaml   ← at least one runnable example Loopfile
```
...
Goose adapter. Your name goes in CONTRIBUTORS.md.
