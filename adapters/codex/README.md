...
Codex (https://github.com/openai/codex) is OpenAI's open-source coding agent. CLI-first, sandboxed execution.

This adapter would let any Loopfile run as a codex workflow, with
...
- **It proves the portability claim.** A Loopfile that runs on both
  Hermes and Codex is the demonstration that INFINI is real.
...
5. Run `infini conformance tests/conformance/ --engine=codex` to verify.
...
└── examples/
    └── codex-loop.yaml   ← at least one runnable example Loopfile
```
...
Codex adapter. Your name goes in CONTRIBUTORS.md.
